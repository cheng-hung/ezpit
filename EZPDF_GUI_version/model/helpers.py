...
def _load_valid_elements():
    ...
    (elem_info/aff_elementonly.txt). Cached after first load. Returns None if
    the file cannot be found, signalling that validation should be skipped."""
    global _VALID_ELEMENTS_CACHE
    if _VALID_ELEMENTS_CACHE is not None:
        return _VALID_ELEMENTS_CACHE
    candidates = [
        os.path.join('elem_info', 'aff_elementonly.txt'),
        os.path.join(os.path.dirname(__file__), '..', 'elem_info', 'aff_elementonly.txt'),
        os.path.join(os.path.dirname(__file__), 'elem_info', 'aff_elementonly.txt'),
    ]
    for path in candidates:
        try:
            ...
            if names:
                _VALID_ELEMENTS_CACHE = set(names)
                return _VALID_ELEMENTS_CACHE
        except OSError:
            continue
    return None  # DB not found -> caller skips existence checking
